...
namespace Reload {
	...
	void loadOffsetJsonFile(const std::string& file) {
		...
		for (auto& [key, value] : weaponJson.items()) {
			for (int i = 0; i < 12; i++) {
				data.rot.arr[i] = value["maxrotation"][i].get<double>();
			}
			data.pos.x = value["slidemax"].get<double>();
			data.pos.y = value["reloadtype"].get<double>();
			data.pos.z = value["ammotype"].get<double>();
	...
	void writeOffsetJson() {
		...
		for (auto& item : g_weaponOffsets->offsets) {
			json weaponJson;
			weaponJson[item.first]["maxrotation"] = item.second.rot.arr;
			weaponJson[item.first]["slidemax"] = item.second.pos.x;
			weaponJson[item.first]["reloadtype"] = item.second.pos.y;
			weaponJson[item.first]["ammotype"] = item.second.pos.z;
